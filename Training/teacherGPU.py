...
class Teacher:
   ...
   def train(self, data, output):
      ...
      if self.dW == None: 
         self.dW = [gpu.zeros(grad.shape) for grad in gradient]
      self.dW = [0.0*params for params in self.dW] 

      # Was there a prior weight update (e.g., for momentum)?
      if self.old_dW == None:
         self.old_dW = [w.copy() for w in self.dW]

      for rate, update in zip(self.update_rates, self.weight_updates):
         for i in range(len(gradient)):
            self.dW[i] += rate*update(gradient=gradient[i], dw=self.dW[i], old_dW=self.old_dW[i], weights=self.model.get_weights()[i])


      # Update the model weights
      self.model.update_weights(self.dW)
